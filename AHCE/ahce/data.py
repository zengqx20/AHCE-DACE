...
def load_and_encode_train_data(num_queries, num_materialized_samples):
    ...
    # file_name_queries = "workloads/scale"
    #file_name_queries = "workloads/synthetic"
    file_name_column_min_max_vals = "data/column_min_max_vals.csv"
    file_name_histogram = "data/histogram_string.csv"

    joins, predicates, tables, samples, label = load_data(file_name_queries, num_materialized_samples)
    hist_file = get_hist_file(file_name_histogram)
    ...
    predicates_hist_train = predicates_hist[:num_train]
    ...
    predicates_hist_test = predicates_hist[num_train:num_train + num_test]
    ...
    train_data = [samples_train, predicates_hist_train, predicates_train, joins_train]
    test_data = [samples_test, predicates_hist_test, predicates_test, joins_test]
...
def make_dataset(samples, predicates_hist, predicates, joins, labels, max_num_joins, max_num_predicates):
    ...
    predicates_hist_masks = []
    predicate_hist_tensors = []
    for predicate_hist in predicates_hist:
        predicate_hist_tensor = np.vstack(predicate_hist)
        num_pad = max_num_predicates - predicate_hist_tensor.shape[0]
        predicate_hist_mask = np.ones_like(predicate_hist_tensor).mean(1, keepdims=True)
        predicate_hist_tensor = np.pad(predicate_hist_tensor, ((0, num_pad), (0, 0)), 'constant')
        predicate_hist_mask = np.pad(predicate_hist_mask, ((0, num_pad), (0, 0)), 'constant')
        predicate_hist_tensors.append(np.expand_dims(predicate_hist_tensor, 0))
        predicates_hist_masks.append(np.expand_dims(predicate_hist_mask, 0))
    predicate_hist_tensors = np.vstack(predicate_hist_tensors)
    predicate_hist_tensors = torch.FloatTensor(predicate_hist_tensors)
    predicates_hist_masks = np.vstack(predicates_hist_masks)
    predicates_hist_masks = torch.FloatTensor(predicates_hist_masks)
    ...
    #predicate_tensors = torch.cat([predicate_tensors, predicate_hist_tensors], dim=2)
    ...
    return dataset.TensorDataset(sample_tensors, predicate_hist_tensors, predicate_tensors, join_tensors, target_tensor, sample_masks, predicates_hist_masks,
